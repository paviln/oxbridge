...
const findMyShips = (req: Request, res: Response) => {
    ...
    Authorize(req, res, "all", function (err: any, decodedUser: any) {
        ...
        Ship.find({ emailUsername: decodedUser.id }, { _id: 0, __v: 0 }, null, function (err: any, ships: IShip[]) {
            if (err)
                return res.status(500).send({ message: err.message || "Some error occurred while retriving ships" });
...
const findAll = (req: Request, res: Response) => {
    ...
    // Checking if authorized 
    Authorize(req, res, "all", function (err: any) {
        if (err)
            return err;
