...
import RacePoint, {IPoint} from '../models/point';
...
const create = async (req: Request, res: Response) => {
  ...
  const event = Event.findOne({_id: location._id});
  if (event) {
    throw new Error('Event id ' + location._id + ' already exists.');
  }
...
// Checks that all foreignkeys are valid. Creates and save a new Location.
const createLocation = async (location: any, res: any) => {
  validateForeignKeys(location);

  location.locationTime.setHours(location.locationTime.getHours()+2);
  const newLocation = await location.save();

  res.status(201).json(newLocation);
};

// Updates racePoint number
function CheckRacePoint(registration: any, res: any, callback: any) {
  Registration.findOne({eventRegId: registration.eventRegId}, function(err: any, eventRegistration: any) {
    if (err) {
    ...
    Location.findOne({eventRegId: registration.eventRegId, {sort: {'locationTime': -1}}, function(err: any, locationRegistration: any) {
      if (err) {
      ...
      if (eventRegistration) {
        Event.findOne({eventId: eventRegistration.eventId}, {_id: 0, __v: 0}, function(err: any, event: any) {
          if (err) {
          ...
          if (event && event.isLive) {
            ...
            RacePoint.findOne({eventId: eventRegistration.eventId, racePointNumber: nextRacePointNumber}, {_id: 0, __v: 0}, function(err: any, nextRacePoint: any) {
              if (err) {
...
// Finds the team distance to the racepoint
const FindDistance = (registration: any, racePoint: any, callback: any) => {
  let checkPoint1 = {
...
// Calculates the closets distance from the team to the checkpoint
...
let pending = 0;
const getLive = (req: any, res: any) => {
  EventRegistration.find({eventId: req.params.eventId}, {_id: 0, __v: 0}, function(err: any, eventRegistrations: any) {
    if (err) {
...
const getScoreboard = (req: any, res: any) => {
  let pending = 0;
...
const getReplay = await (req: Request, res: Response) => {
  Registration.find({_id: req.params.eventId}, {_id: 0, __v: 0}, function(err, eventRegistrations) {
    if (err) {
      return res.status(500).send({message: err.message || 'Some error occurred while retriving eventRegistrations'});
    }
...
// Deleting all locations with an given id
const deleteFromEventRegId = async (req: Request, res: Response) => {
  const locations = await Location.deleteMany({_id: req.params.eventId});
  if (!locations) {
    throw new NotFound('LocationRegistrations not found with eventRegId ' + req.params.regId);
  }
...
const validateForeignKeys = async (registration: any) => {
  ...
  const location = await Location.findOne({_id: registration.eventRegId});
  if (!location) throw new Error('EventRegistration with id ' + registration.eventRegId + ' was not found.');
}
